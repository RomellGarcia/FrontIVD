...
const Vision = () => {
  ...
  const fetchVisiones = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('http://localhost:5000/api/contenido/vision');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        response = await axios.put(`http://localhost:5000/api/contenido/vision/${editingId}`, payload);
        ...
        response = await axios.post('http://localhost:5000/api/contenido/vision', payload);
        MySwal.fire({
          title: '¡Éxito!',
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      setLoading(true);
      try {
        const response = await axios.delete(`http://localhost:5000/api/contenido/vision/${id}`);
